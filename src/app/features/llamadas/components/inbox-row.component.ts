    ...
    >
      ...
      <div style="flex: 1; min-width: 0;">
        <div style="display: flex; align-items: center; gap: 8px;">
          ...
          <app-tipo-tag [tipo]="llamada().tipo" [subcategoria]="llamada().subcategoria" />
        ...
        >
          <span style="color: #334155; font-weight: 500;">{{ llamada().empresa }}</span> · {{ llamada().resumen }}
        </div>
        <div style="display: flex; gap: 8px; margin-top: 6px; align-items: center;">
          <app-estado-tag [estado]="llamada().estado" />
          <span style="font-size: 11px; color: #64748b;">{{ llamada().agenteId }}</span>
...
export class InboxRowComponent {
  ...
  initials = computed<string>(() => {
    const c = this.llamada().cliente;
    if (c === 'Desconocido') return '?';
    return c.split(' ').map((w) => w[0]).slice(0, 2).join('');
  });
}
